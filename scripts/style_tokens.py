...
def test_ranges(tokens):
    """Locate inline Rust modules or imports guarded only for tests."""
    linked, _ = pairs(tokens)
    ranges = []
    for index, token in enumerate(tokens):
        if token.text != "mod":
            continue
        opening = next((pos for pos in range(index + 1, min(index + 8, len(tokens)))
                        if tokens[pos].text == "{"), None)
        prior = tokens[max(0, index - 14):index]
        if opening in linked and any(item.text == "cfg" for item in prior) \
                and any(item.text == "test" for item in prior):
            ranges.append((opening, linked[opening]))
    return ranges
...
def rust_wildcards(tokens):
    ...
    for index, token in enumerate(tokens):
        ...
        prior = tokens[max(0, index - 14):index]
        guarded |= any(item.text == "cfg" for item in prior) and any(item.text == "test" for item in prior)
