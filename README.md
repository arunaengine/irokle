...
`FjallStorage` stores schema version 7. Opening a database at schema 1 to 6 migrates it in place before the store is used. The steps up to schema 6 each run in one transaction that checks the version again, so concurrent or repeated opens are safe. Acknowledgements from before schema 2 stay stored but are uncertified. Schema 4 staging is discarded; schema 5 namespaces gain their revision and bytes, and a schema 5 activation interrupted after its copies resumes with the copies hidden. Schema 7 stores each observed clock by the hash of its trie root instead of every entry: one transaction writes version 7 together with a migration cursor, then bounded transactions of at most 1,024 records rewrite the metadata of the main keyspace and of every staging keyspace, and the last step removes the cursor. An open interrupted during that rewrite leaves version 7 and the cursor; the next open continues from the cursor. A binary that knows only schema 6 or older refuses a version 7 database, including one still migrating, so going back needs a copy taken before the upgrade. A database with a newer, unknown schema version is refused with `Error::Storage`.
