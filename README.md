...
`FjallStorage` stores schema version 6. Opening a database at schema 1 to 5 migrates it in place, one transaction per step, and each step checks the version again, so concurrent or repeated opens are safe. Acknowledgements from before schema 2 stay stored but are uncertified. Schema 4 staging is discarded; schema 5 namespaces gain their revision and bytes, and a schema 5 activation interrupted after its copies resumes with the copies hidden. A database with a newer, unknown schema version is refused.
