...
//! A transport's page loop through the public API: request and page repeat until
//! the page reports no more.
...
#[test]
fn pages_until_current() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(pages, 3);
    assert_eq!(
        bob.storage().actor_clock(&topic.id())?,
        alice.storage().actor_clock(&topic.id())?
    );
    Ok(())
}
